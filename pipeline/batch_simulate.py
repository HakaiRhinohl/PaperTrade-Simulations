...
Parameter grid (fully crossed):
  ...
  rateMult fixed at 15000 (Rollbit's published value, via DEFAULTS)
  posMult fixed at 1,384,615,385 BTC / 923,076,923 ETH (Rollbit's published
  1384.615 / 923.077, quoted in $-millions, via DEFAULTS)
...
Usage:
    python3 batch_simulate.py                       # uses data/max_lev_trades_v3.parquet
...
def _run_one(scenario: dict) -> dict:
    ...
    summary = {
        ...
        "finalFrontend": round(s.get("finalFrontend", 0.0), 2),
...
def main():
    ...
    parser.add_argument("--fixed-impact", action="store_true",
                        help=(
                            "Collapse the impact-param sweep (base_rate, BTC/ETH reference "
                            "notional) to the single fixed values in DEFAULTS. Keeps the "
                            "flow x startDay x ADL x emission axes, so the batch drops from "
                            "27,360 to 608 scenarios. Use for emission/flow charts when the "
                            "impact params are fixed and there is nothing to sweep."
                        ))
    ...
    if args.fixed_impact:
        global BASE_RATE_VALUES, BTC_REF_VALUES, ETH_REF_VALUES
        BASE_RATE_VALUES = [DEFAULTS["btcBaseRate"]]
        BTC_REF_VALUES   = [int(DEFAULTS["btcReferenceNotional"])]
        ETH_REF_VALUES   = [int(DEFAULTS["ethReferenceNotional"])]
        logging.info("FIXED-IMPACT mode: base_rate=%s, btc_ref=%s, eth_ref=%s "
                     "(impact-param sweep collapsed to 1 combo)",
                     BASE_RATE_VALUES[0], BTC_REF_VALUES[0], ETH_REF_VALUES[0])
