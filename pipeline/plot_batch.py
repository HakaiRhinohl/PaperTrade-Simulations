...
from local_server import DEFAULTS
...
PARAM_DEFAULTS = {"btcBaseRate": DEFAULTS["btcBaseRate"],
                  "btcReferenceNotional": DEFAULTS["btcReferenceNotional"],
                  "ethReferenceNotional": DEFAULTS["ethReferenceNotional"]}
...
def usd_fmt(x, _):
    ...
    if abs(x) >= 1e4: return f"${x/1e3:.0f}K"      # >= $10K
    if abs(x) >= 1e3: return f"${x/1e3:.1f}K"      # $1K–$10K: one decimal
    return f"${x:,.0f}"                            # below $1K: plain dollars
...
def usd_signed(x, _):
    ...
    if v >= 1e4: return f"{s}${v/1e3:.0f}K"
...
def plot_07_lp_paths(results, daily_lp, out: Path, suffix="", emission=False, adl=False):
    ...
    # count launch dates whose LP never reaches the $5M cap within the window
    n_never = 0
    for _, row in subset.iterrows():
        sid = row["scenario_id"]
        if sid in daily_lp.index and np.nanmax(daily_lp.loc[sid].values.astype(float)) < 5_000_000 - 1:
            n_never += 1
    never_note = f"  ·  {n_never} launch date(s) never reach $5M (dashed red)" if n_never else "  ·  every launch date fills the $5M cap"
    title_base = f"LP recovery from every launch date ({adl_tag}){_em_label(emission)}{never_note}"
    ...
    def _plot_lines(ax_):
        ...
        for i, (_, row) in enumerate(subset.iterrows()):
            ...
            if np.nanmax(lp_vals) < 5_000_000 - 1:
                lbl = "never reaches $5M" if not seen_never else None
                seen_never = True
                ax_.plot(dates, lp_vals, color="#d62728", lw=1.4, alpha=0.9, ls="--",
                         label=lbl, zorder=6)
            else:
    ...
    ax2.legend(fontsize=7, ncol=3, loc="lower right", framealpha=0.9)
    fig2.tight_layout()
...
def plot_lp01_header_stats(results_all, out: Path):
    ...
    Single table (queue/debt stats are identical across emission modes).
    """
    flows    = sorted(results_all["sampleFraction"].unique())
    ...
    fig, (ax_banner, ax_tbl) = plt.subplots(2, 1, figsize=(14, 8),
                                             gridspec_kw={"height_ratios": [1.2, 3.0]})
    ...
    for label, fn in row_specs:
        row = []
        for fl in flows:
            d = results_all[np.isclose(results_all["sampleFraction"], fl)]
            row.append(fn(d) if not d.empty else "–")
        row.append(fn(results_all))
    ...
    n_combos = results_all[["btcBaseRate", "btcReferenceNotional",
                            "ethReferenceNotional"]].drop_duplicates().shape[0]
    n_start = results_all["startDay"].nunique()
    combo_word = "impact-param combo" + ("s" if n_combos != 1 else "")
    fig.suptitle(
        "LP queue / debt stats — full parameter grid\n"
        f"Columns = flow rate buckets aggregated across {n_combos} {combo_word} "
        f"x {n_start} start days x 2 ADL modes x 2 emission models",
...
def plot_lp02_debt_timing(results_all, out: Path):
    ...
    Single chart (debt/queue stats are identical across emission modes).
    """
    flows      = sorted(results_all["sampleFraction"].unique())
    ...
    subset = results_all[base_mask]
    if "emissionBased" in results_all.columns:
        subset = subset[subset["emissionBased"] == False]
    ...
    # annotate the single largest debt event
    if not has_debt.empty:
        mx = has_debt.loc[has_debt["maxDebt"].idxmax()]
        ax.annotate(f"max ${mx['maxDebt']:,.2f}\n{fl_labels[mx['sampleFraction']]}, "
                    f"launch d{int(mx['startDay'])}",
                    xy=(day_to_date(int(mx["startDay"])), mx["maxDebt"]),
                    xytext=(-10, -30), textcoords="offset points", fontsize=8, ha="right",
                    arrowprops=dict(arrowstyle="->", color="#555", lw=1))
    ax.set_title(f"Max outstanding debt by launch date  (base model, ADL off — "
                 f"{len(subset):,} of 608 scenarios)\n"
...
def plot_lp03_days_to_thresholds(results_all, daily_lp_all, out: Path, adl=False):
    ...
    fig, axes = plt.subplots(len(em_modes), len(thresholds),
                             ...
                             sharey=False)   # $2M and $5M have different scales
    if len(em_modes) == 1:
        axes = axes.reshape(1, -1)
    fig.suptitle(
        f"Days from launch until LP first crosses $2M / $5M  ({adl_tag})\n"
        "One scenario per launch date  ·  ▲ = off the top (value shown)  ·  "
        "✖ = never reached inside the 290-day window",
    ...
    for row_i, em in enumerate(em_modes):
        ...
        for col_i, (thresh, thresh_label) in enumerate(thresholds):
            ...
            # visual ceiling: 97th pct of finite values, rounded up (min 15d)
            finite = subset2["days_to"].dropna().values
            ceil = max(15.0, np.ceil(np.percentile(finite, 97) / 5.0) * 5.0) if len(finite) else 15.0

            for fl in flows:
                fs = subset2[np.isclose(subset2["sampleFraction"], fl)].sort_values("startDay")
                x = np.array([day_to_date(int(sd)) for sd in fs["startDay"]])
                y = fs["days_to"].values.astype(float)
                # in-range line (off-scale + never-reached broken to NaN so the line gaps)
                y_line = np.where(np.isfinite(y) & (y <= ceil), y, np.nan)
                ax.plot(x, y_line, color=fl_colors[fl], lw=1.8, label=f"{fl_labels[fl]} flow")
                # off-scale: finite but above the ceiling → ▲ pinned to the top + value
                off = np.isfinite(y) & (y > ceil)
                for xi, yi in zip(x[off], y[off]):
                    ax.plot(xi, ceil, marker="^", color=fl_colors[fl], ms=7, clip_on=False, zorder=5)
                    ax.annotate(f">{int(yi)}d", (xi, ceil), textcoords="offset points",
                                xytext=(0, 5), fontsize=6, color=fl_colors[fl], ha="center")
                # never reached (NaN) → ✖ at the top, distinct from off-scale
                for xi in x[~np.isfinite(y)]:
                    ax.plot(xi, ceil, marker="x", color=fl_colors[fl], ms=6, mew=1.6,
                            clip_on=False, zorder=5)

            ax.set_ylabel("Days from launch")
            ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{int(v)}d"))
            ax.set_ylim(0, ceil)
            dates_axis(ax)
...
def plot_lp04_debt_by_flow(results_all, out: Path):
    ...
    fig.suptitle(
        ...
        "One scenario per launch date · official parameters · all four ADL × volume cells",
        fontsize=13, fontweight="bold", y=1.0
    )
    ...
    for em in em_modes:
        for adl in adl_modes:
            ...
            # one scenario per (flow, launch date) → plot the value directly, no band
            for fl in flows:
                d = subset[np.isclose(subset["sampleFraction"], fl)].sort_values("startDay")
                if d.empty:
                    continue
                x_dates = [day_to_date(int(sd)) for sd in d["startDay"]]
                ax_debt.plot(x_dates, d["maxDebt"].values, color=fl_colors[fl],
                             lw=1.8, alpha=0.9, label=fl_labels[fl])
                ax_q.plot(x_dates, d["maxQueueLen"].values, color=fl_colors[fl],
                          lw=1.8, alpha=0.9, label=fl_labels[fl])
