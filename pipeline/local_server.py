...
DEFAULTS = dict(
    ...
    stakerPct=0.01, frontendPct=0.01, lpCapUsd=5_000_000.0, stakedFraction=1.0,
    sampleFraction=1.0, volatility=1.0, volumeScale=1.0,
    btcBaseRate=0.10, btcRateMultiplier=15_000.0,
    btcPositionMultiplier=1_384_615_385.0, btcReferenceNotional=100_000.0,
    ethBaseRate=0.10, ethRateMultiplier=15_000.0,
    ethPositionMultiplier=923_076_923.0, ethReferenceNotional=100_000.0,
    deadbandBps=0.2,   # half-spread deadband (bps): winning move reduced by this
    adlWorstCase=False,
    startDay=0,
...
def simulate(trades: dict, params: dict) -> dict:
    ...
    frontend_pct = p.get("frontendPct", 0.0)
    deadband = p.get("deadbandBps", 0.0) / 10_000.0   # half-spread, as a move fraction
    lp_cap = p["lpCapUsd"]
    ...
    for day in range(SIM_DAYS):
        if day < start_day:
            ...
            state_out.append({"day": day, "lpBalance": lp_balance,
                "paperSupply": 0, "stakers": 0, "frontend": 0, "tailProgress": 0,
                "btcLp": 0, "ethLp": 0})
            continue
        day_n_trades = 0
        day_n_liq = 0
        ...
        for idx in by_day[day]:
            ...
            if paper_liq:
                ...
                if net_m <= 0.0:
                    ...
                    adjusted = raw_pnl * (net_m / m) * impact_scale(ibtc, net_m, p)
            ...
            if lp_event > 0:
                ...
                # Protocol slice: staker + frontend each take their pct up front;
                # the rest grows the LP (and post-cap overflow routes to stakers).
                stakers_cut  = lp_event * staker_pct
                frontend_cut = lp_event * frontend_pct
                frontend_total += frontend_cut
                lp_gain = lp_event - stakers_cut - frontend_cut
                ...
                # FIX 3: impact-revenue fee (suppressed when queue non-empty),
                # split between stakers and frontend by their pcts.
                staker_impact = 0.0
                frontend_impact = 0.0
                ...
                if lp_balance >= payout:
                    lp_balance -= payout
                    # Staker + frontend cut from impact revenue (from LP's retained amount)
                    total_impact = staker_impact + frontend_impact
                    if total_impact > 0:
                        actual_impact = min(total_impact, lp_balance)
                        lp_balance -= actual_impact
                        stakers_total  += actual_impact * (staker_impact / total_impact)
                        frontend_total += actual_impact * (frontend_impact / total_impact)
                else:
