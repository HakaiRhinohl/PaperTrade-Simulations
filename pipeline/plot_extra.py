...
def main():
    ...
    parser.add_argument("--trades", type=Path,
                        default=Path("data") / "max_lev_trades_v3.parquet")
    parser.add_argument("--results", type=Path, default=Path("batch_results.csv"))
    ...
    if not results_path.exists():
        print(f"  (skipping extra charts — {results_path} not found; run with --sweep)")
        return
