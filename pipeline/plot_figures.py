...
def _scenario_id(flow: float, em: bool = False) -> str:
    ...
    return (f"br={DEFAULTS['btcBaseRate']}"
            f"_btcRN={int(DEFAULTS['btcReferenceNotional'])}"
            f"_ethRN={int(DEFAULTS['ethReferenceNotional'])}"
            f"_flow={flow_pct}%_start=d0_adl=off_{em_tag}")
...
def plot_f04(daily_all):
    ...
    ax.text(0.5, 1.02, E(f"Default params: baseRate {DEFAULTS['btcBaseRate']:.0%}, "
                         f"btcRef ${int(DEFAULTS['btcReferenceNotional'])//1000}K, "
                         f"ethRef ${int(DEFAULTS['ethReferenceNotional'])//1000}K, ADL off"),
...
def plot_f09(daily_all):
    ...
    fig.suptitle(E("Cumulative fees per token for a day-0 staker who holds"),
                 fontsize=14, fontweight="bold", y=0.99)
    ...
    for idx, fl in enumerate(FLOWS):
        ...
        if idx == 0:
            ax.plot([], [], color="gray", ls="--", lw=0.8, alpha=0.4,
                    label="Naïve: total fees / final supply (dashed)")
        ax.set_ylabel(E("Cumulative fees per staked token ($)"))
...
def plot_f13(daily_all):
    ...
    for idx, fl in enumerate(FLOWS):
        ...
        mask = (np.isclose(batch["sampleFraction"], fl) &
                np.isclose(batch["btcBaseRate"], DEFAULTS["btcBaseRate"]) &
                np.isclose(batch["btcReferenceNotional"], DEFAULTS["btcReferenceNotional"]) &
                np.isclose(batch["ethReferenceNotional"], DEFAULTS["ethReferenceNotional"]) &
...
def plot_f15():
    ...
    param_mask = (np.isclose(batch["btcBaseRate"], DEFAULTS["btcBaseRate"]) &
                  np.isclose(batch["btcReferenceNotional"], DEFAULTS["btcReferenceNotional"]) &
                  np.isclose(batch["ethReferenceNotional"], DEFAULTS["ethReferenceNotional"]))
