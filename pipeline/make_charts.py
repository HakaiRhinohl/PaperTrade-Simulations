...
def chart_mint_curve(cfg, png_path=None):
    ...
    def rate_at(lp):
        # PAPER minted per $1: flat below the $2M threshold, tail decay above it
        lp = np.asarray(lp, dtype=float)
        H  = np.maximum(0.0, lp - threshold)
        return np.where(lp < threshold, flat_rate, flat_rate * (S / (S + H)) ** 2)

    LINE_GREEN = "#2e8b57"                     # green line (matches original)
    CREAM      = "#fdf6ec"                      # cream background
    x_max = 105_000_000                         # start $0, run through the $100M / 30-per-$ point
    lp = np.linspace(0, x_max, 800)
    rate = rate_at(lp)

    fig, ax = plt.subplots(figsize=(11, 4.6))
    ax.set_facecolor(CREAM)
    fig.patch.set_facecolor("white")
    ax.plot(lp, rate, color=LINE_GREEN, linewidth=2.2, zorder=3)
    ax.fill_between(lp, 0, rate, color=LINE_GREEN, alpha=0.10)

    # flat -> tail boundary
    ax.axvline(threshold, color="#666", linestyle="--", linewidth=1, alpha=0.7)
    ax.text(threshold + x_max * 0.012, flat_rate * 0.55, "flat → tail ($2M)",
            ha="left", va="center", fontsize=9, color="#444", rotation=90)

    # labelled points: (cumulative LP gain, PAPER per $1)
    pts = [0, 10_000_000, 50_000_000, 100_000_000]
    for x in pts:
        yv = float(rate_at(x))
        ax.scatter([x], [yv], s=45, color=LINE_GREEN, zorder=4,
                   edgecolor="white", linewidth=0.8)
        dx, dy = (4, 10) if x == 0 else (8, 8)
        ax.annotate(f"{int(round(yv))} /$",
                    (x, yv), textcoords="offset points",
                    xytext=(dx, dy), fontsize=9, fontweight="bold", color=LINE_GREEN)

    # region annotations
    ax.annotate("LP=$0", (0, flat_rate), textcoords="offset points",
                xytext=(8, -18), fontsize=9, color="#444")
    ax.text(x_max * 0.60, flat_rate * 0.30, "TAIL (asymptotic decay)",
            ha="center", va="center", fontsize=9, color="#444", style="italic")

    ax.set_title("PAPER minted per dollar of trader loss, by cumulative LP gain")
    ax.set_xlabel("Cumulative tracked LP gain (USD)")
    ax.set_ylabel("PAPER minted per $1 of trader loss")
    ax.set_xlim(0, x_max)                       # start at $0 — no -$5M tick
    ax.set_ylim(0, flat_rate * 1.12)
    ax.set_xticks([0, 10_000_000, 25_000_000, 50_000_000, 75_000_000, 100_000_000])
    ax.xaxis.set_major_formatter(mtick.FuncFormatter(fmt_dollars))
    return fig_save(fig, png_path)
