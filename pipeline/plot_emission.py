...
def get_defaults(r, emission: bool):
    """Get rows where all impact params are at the canonical (DEFAULTS) values."""
    mask = (np.isclose(r["btcBaseRate"], DEFAULTS["btcBaseRate"]) &
            np.isclose(r["btcReferenceNotional"], DEFAULTS["btcReferenceNotional"]) &
            np.isclose(r["ethReferenceNotional"], DEFAULTS["ethReferenceNotional"]))
    ...
    for ax_idx, (em, title) in enumerate([(False, "Base model"), (True, "Emission model")]):
        ...
        for flow in FLOW_FRACS:
            ...
            ax.plot(x_dates, data["traderNet"], color=FLOW_COLORS[flow],
                    lw=2, alpha=0.85, label=FLOW_LABELS[flow])
            # annotate the earliest-launch (largest-magnitude) value per flow
            y0 = float(data["traderNet"].iloc[0])
            ax.annotate(f"${y0/1e6:.1f}M", (x_dates[0], y0), textcoords="offset points",
                        xytext=(6, 0), fontsize=8, color=FLOW_COLORS[flow],
